...
- **Click an item in the calendar grid, or the pencil next to an agenda/list item** — opens one editor for title, type (Meeting/Event/Reminder/Subscription/Invoice reminder/Task), date, start/end time, an "All day" checkbox, repeat rule, an advance reminder lead time and (for a Meeting) client, all in one place, without opening the note itself. Cost only shows once Subscription or Invoice reminder is picked, or when editing something that already has one set. Changing the title renames the file; changing the type updates its tag, status, client and folder together. Converting to or from a real Invoice isn't offered — editing an existing Invoice's own title or schedule still works, its type just shows as a locked label.
